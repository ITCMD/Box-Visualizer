...
:done
echo Completed.
echo Changelog Not Availalbe on Bget Version.
pause
:StartFile
set _Color=0a
...
if exist "%temp%\AddEcho.bat" goto 27925132615643131482315223518 
echo @echo off>"%temp%\AddEcho.bat"
echo for /f "usebackq tokens=*" %%%%A in ("%%~1") do (echo echo %%%%A^>^>%%~2) >>"%temp%\AddEcho.bat"
echo exit /b >>"%temp%\AddEcho.bat"
:27925132615643131482315223518 
if not exist "%temp%\AddEcho.bat" exit /b 2
echo %~1|find "\" >nul
...
"%temp%\AddEcho.bat" "temp.txt" "%output%"
echo :%num% >> %output%
